...
let fiERR = document.getElementById("Error");
let laerr = document.getElementById("laERR");
...
fname.addEventListener("input", () => {
    if (fname.value.length < 3) {
        fiERR.textContent = "firts name required min. 3 character";
        valid = false;
    } else {
        fiERR.textContent = "";
    }
...
fname.addEventListener("input", () => {
    if (fname.value.length > 100) {
        console.error();
        error1.textContent = "firts name required max. 100 char";
        valid = false;
    } else {
        error1.textContent = "";
    }
...
lanme.addEventListener("input", () => {
    if (lanme.value.length < 3) {
        error2.textContent = "last name required min. 3 character.";
        valid = false;
    } else {
        error2.textContent = "";
    }
...
lanme.addEventListener("input", () => {
    if (lanme.value.length > 100) {
        console.error();
        laerr.textContent = "firts name required max. 100 char";
        valid = false;
    } else {
        laerr.textContent = "";
    }
...
age1.addEventListener("input", () => {
    const value = parseInt(age1.value);
    ...
    if (value < 20) {
        ...
        showToast("⚠️ Above the age of 80 is not allowed!");
    }
});
...
form.addEventListener("submit", (e) => {
    ...
    if (value < 20 || value > 80) {
        e.preventDefault();
        // alert.textContent =
        // "Please fix the errors in age field before submitting";
        ageErr1.textContent = "age must be between 20 to 80";
        age1.focus();
        age1.select();
    }
});
